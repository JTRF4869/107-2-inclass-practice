...
major <- c("經濟系","法律系","社會系","經濟系")
grade <- c(50,60,92,88)
meanEcon <- sum(as.integer(fromEcon)*grade)/sumEcon
meanEcon
```
...
```{r ans23}
lateTime <- c("1","2","3","4","5","6")
nightOwl <- hour(twTime) %in% lateTime
nightOwl
```
### 2.4
承上題，在上題半夜間投票的學生佔總投票人數的比何為多少？（存在答案物件nightOwlVoteShare）（hint: `length()`可以計算向量的元素個數）
```{r ans24}
sum(nightOwl)/length(nightOwl)
```
...
```{r ans31}
Passgrade <- grade >= 60
as.integer(Passgrade)*credit -> earnedCredit
earnedCredit
```
...
```{r ans32}
sum(as.integer(studentID=="046S6f645"&schoolYear=="100")*credit) ->credit100ChosenStudent
...
```{r ans33}
sum(as.integer(studentID=="046S6f645"&Passgrade&schoolYear=="100")*credit) ->earnedCredit100ChosenStudent
...
```{r ans34}
sum((as.integer(studentID=="046S6f645"&schoolYear=="100")*grade)*credit)/credit100ChosenStudent ->gpa100ChosenStudent
gpa100ChosenStudent
```
